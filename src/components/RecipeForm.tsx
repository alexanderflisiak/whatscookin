...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Batch database operations to avoid N+1 query bottlenecks
      ...
      if (validIngredients.length > 0) {
        ...
        // 2. Identify and bulk insert missing ingredients
        const missingNames = ingNames.filter(name => !existingMap.has(name))
        ...
        // 3. Bulk insert bridge records
        const bridgeRecords = validIngredients.map(ing => ({
          recipe_id: recipeId,
          ingredient_id: existingMap.get(ing.name.trim().toLowerCase()),
          amount: ing.amount || null,
          unit: ing.unit || null
        })).filter(record => record.ingredient_id) // Ensure ingredient_id was successfully found/created
