...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Optimize N+1 queries by batching ingredient lookups and inserts
      const validIngredients = data.ingredients.filter(ing => ing.name).map(ing => ({
        ...ing,
        name: ing.name.trim().toLowerCase()
      }))

      if (validIngredients.length > 0) {
        const ingNames = [...new Set(validIngredients.map(ing => ing.name))]
        ...
        const bridgeInserts = validIngredients.map(ing => ({
          recipe_id: recipeId,
          ingredient_id: existingMap.get(ing.name),
          amount: ing.amount || null,
          unit: ing.unit || null
        })).filter(insert => insert.ingredient_id)
