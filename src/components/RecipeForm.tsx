...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // Clean and collect valid ingredients from the form
      ...
      if (validIngredients.length > 0) {
        const uniqueIngredientNames = [...new Set(validIngredients.map(ing => ing.name))]
        ...
        const existingMap = new Map((existingIngredients || []).map(ing => [ing.name, ing.id]))

        // 2. Identify and insert missing ingredients
        const missingNames = uniqueIngredientNames.filter(name => !existingMap.has(name))

        if (missingNames.length > 0) {
          const { data: newlyInsertedIngredients } = await supabase
            .from('ingredients')
            .insert(missingNames.map(name => ({ name })))
            .select('id, name')

          if (newlyInsertedIngredients) {
            newlyInsertedIngredients.forEach(ing => {
              existingMap.set(ing.name, ing.id)
            })
          }
        }

        // 3. Batch insert recipe_ingredients bridge records
        const bridgeRecords = validIngredients
          .map(ing => {
            const ingredientId = existingMap.get(ing.name)
