...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // Batch fetching and inserting ingredients
      ...
      if (validIngredients.length > 0) {
        ...
        if (missingNames.length > 0) {
          const { data: newIngs } = await supabase.from('ingredients').insert(missingNames.map(name => ({ name }))).select('id, name')
          if (newIngs) {
        ...
        const bridgePayloads = validIngredients.map(ing => {
          const ingName = ing.name.trim().toLowerCase()
          return {
            recipe_id: recipeId,
            ingredient_id: existingMap.get(ingName),
