...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Batch database operations to fix N+1 query issue
      // O(N) loop converted to O(1) batched calls for ingredients find-or-create
      ...
      if (validIngredients.length > 0) {
        ...
        // 1. Fetch existing ingredients in one batch
        ...
        // 2. Insert missing ingredients in one batch
        ...
        // 3. Insert bridge records in one batch
