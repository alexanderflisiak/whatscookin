...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // Instead of looping and sequentially querying/inserting each ingredient (O(n) queries),
      // we batch fetch, batch insert missing ones, and batch insert bridge records (3 queries total).
      ...
      if (validIngredients.length > 0) {
        ...
        // Batch 1: Fetch existing ingredients
        const { data: existingIngs } = await supabase
          .from('ingredients')
          .select('id, name')
          .in('name', ingredientNames)

        const existingMap = new Map((existingIngs || []).map(i => [i.name, i.id]))

        // Batch 2: Insert missing ingredients
        const missingNames = [...new Set(ingredientNames.filter(name => !existingMap.has(name)))]
        ...
        // Batch 3: Bulk insert bridge records
