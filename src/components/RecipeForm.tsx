...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // BOLT OPTIMIZATION: ⚡ Batch ingredient resolution to avoid N+1 queries.
      // Instead of querying and inserting per ingredient sequentially,
      // fetch all existing ingredients, insert any missing in one batch,
      // and then insert all recipe_ingredients in one batch.
      const validIngredients = data.ingredients.filter(ing => ing.name && ing.name.trim() !== '')

      if (validIngredients.length > 0) {
        // Collect normalized ingredient names
        const ingNames = Array.from(new Set(validIngredients.map(ing => ing.name.trim().toLowerCase())))
        
        // Fetch existing ingredients in one batch
        const { data: existingIngs } = await supabase
          .from('ingredients')
          .select('id, name')
          .in('name', ingNames)

        const ingredientMap = new Map((existingIngs || []).map(ing => [ing.name, ing.id]))
        const missingIngNames = ingNames.filter(name => !ingredientMap.has(name))

        // Batch insert missing ingredients
        if (missingIngNames.length > 0) {
          const newIngsPayload = missingIngNames.map(name => ({ name }))
          const { data: newlyCreatedIngs, error: newIngsErr } = await supabase
            .from('ingredients')
            .insert(newIngsPayload)
            .select('id, name')

          if (!newIngsErr && newlyCreatedIngs) {
            newlyCreatedIngs.forEach(ing => ingredientMap.set(ing.name, ing.id))
          }
        }

        // Build batch payload for recipe_ingredients
        const recipeIngredientsPayload = validIngredients
          .map(ing => {
        ...
        if (recipeIngredientsPayload.length > 0) {
          await supabase.from('recipe_ingredients').insert(recipeIngredientsPayload)
        }
      }
