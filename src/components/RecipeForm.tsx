...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Prevent N+1 query bottleneck by extracting unique ingredient names
      // and performing batched `.in()` and `.insert([])` queries
      const uniqueIngs = Array.from(new Map(
        data.ingredients
      ...
      if (uniqueIngs.length > 0) {
        const ingNames = uniqueIngs.map(i => i.name.trim().toLowerCase())
        ...
        const existingIngMap = new Map((existingIngs || []).map(i => [i.name, i.id]))
        ...
        if (missingNames.length > 0) {
          ...
          ;(newIngs || []).forEach(i => existingIngMap.set(i.name, i.id))
        }

        // Batch insert recipe_ingredients
        const recipeIngredientsPayload = data.ingredients
          .filter(i => i.name?.trim())
          .map(ing => {
