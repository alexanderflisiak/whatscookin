...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Batch database operations to avoid N+1 query bottleneck
      const validIngredients = data.ingredients
        .map(ing => ({ ...ing, name: ing.name?.trim().toLowerCase() }))
        .filter(ing => ing.name)
        
      if (validIngredients.length > 0) {
        const ingredientNames = Array.from(new Set(validIngredients.map(i => i.name)))
        ...
        const missingNames = ingredientNames.filter(name => !existingIngMap.has(name))
        ...
        // 3. Batch insert recipe_ingredients
        const recipeIngredientsToInsert = validIngredients
          .map(ing => {
            const ingredientId = existingIngMap.get(ing.name)
