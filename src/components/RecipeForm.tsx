  ...
  return (
    <div className="max-w-3xl mx-auto p-4 pb-24">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <fieldset className="relative pt-6">
          <legend className="absolute top-0 left-0 block text-sm font-semibold mb-1 w-full flex justify-between items-center">
            <span>Ingredients</span>
            <button 
              type="button" 
              onClick={() => append({ amount: '', unit: '', name: '' })}
              className="text-text-hi text-sm flex items-center gap-1 font-medium bg-stone-50 px-2 py-1 rounded focus-visible:ring-2 focus-visible:ring-stone-400 outline-none"
            >
            ...
            {fields.map((field, index) => (
              <div key={field.id} className="flex gap-2 items-start">
                ...
                <button 
                  ...
                  className="p-2 text-text-lo hover:text-danger hover:bg-red-50 rounded-lg transition-colors mt-0.5 shrink-0 focus-visible:ring-2 focus-visible:ring-red-400 outline-none"
                >
