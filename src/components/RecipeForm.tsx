...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Batch database operations to fix N+1 query issue for ingredients
      ...
      if (uniqueNames.length > 0) {
        ...
        if (missingNames.length > 0) {
          const { data: newIngs } = await supabase.from('ingredients')
            ...
            .select('id, name')
          if (newIngs) allIngredients = [...allIngredients, ...newIngs]
        }

        // Create lookup map
        const ingMap = new Map(allIngredients.map(i => [i.name, i.id]))
