...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Fix N+1 query problem by batching find-or-create ingredient operations
      const validIngredients = data.ingredients.filter(ing => ing.name?.trim())
      if (validIngredients.length > 0) {
        const uniqueIngNames = [...new Set(validIngredients.map(ing => ing.name.trim().toLowerCase()))]
        
        // Batch select existing ingredients
        const { data: existingIngs } = await supabase.from('ingredients').select('id, name').in('name', uniqueIngNames)
        const existingIngMap = new Map((existingIngs || []).map(ing => [ing.name, ing.id]))

        const missingNames = uniqueIngNames.filter(name => !existingIngMap.has(name))
        ...
        const recipeIngredientsData = validIngredients.map(ing => {
          const ingName = ing.name.trim().toLowerCase()
          return {
            recipe_id: recipeId,
            ingredient_id: existingIngMap.get(ingName)!,
