  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      if (validIngredients.length > 0) {
        // ⚡ Bolt: Batch database operations to solve N+1 problem on form save
        const uniqueIngredientNames = Array.from(new Set(validIngredients.map(ing => ing.name.trim().toLowerCase())))
        
        // Bulk fetch existing ingredients
        const { data: existingIngs } = await supabase
          .from('ingredients')
          .select('id, name')
          .in('name', uniqueIngredientNames)

        const ingredientMap = new Map((existingIngs || []).map(ing => [ing.name.toLowerCase(), ing.id]))

        // Find missing ingredients and bulk insert them
        const missingNames = uniqueIngredientNames.filter(name => !ingredientMap.has(name))
        ...
        if (missingNames.length > 0) {
          ...
          if (newIngs) {
            newIngs.forEach(ing => ingredientMap.set(ing.name.toLowerCase(), ing.id))
          }
        }
        ...
        }).filter(record => record.ingredient_id) // Safety check
