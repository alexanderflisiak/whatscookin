...
export function RecipeForm({ initialData, mode = 'create' }: { initialData?: Partial<Recipe> & { ingredients?: any[] }, mode?: 'create' | 'edit' }) {
  ...
  async function onSubmit(data: RecipeFormData) {
    ...
    try {
      ...
      // ⚡ Bolt: Batch database operations to fix N+1 query bottleneck
      const validIngredients = data.ingredients.filter(ing => ing.name?.trim())

      if (validIngredients.length > 0) {
        // 1. Deduplicate and fetch existing ingredients in a single query
        ...
        // Create a lookup map with normalized names
        const ingMap = new Map((existingIngs || []).map(ing => [ing.name.toLowerCase(), ing.id]))

        // 2. Identify missing ingredients and bulk insert them
        const missingNames = uniqueNames.filter(name => !ingMap.has(name))

        if (missingNames.length > 0) {
          const newIngPayloads = missingNames.map(name => ({ name }))
          const { data: newIngs, error: newIngErr } = await supabase
            .from('ingredients')
            .insert(newIngPayloads)
            .select('id, name')

          if (!newIngErr && newIngs) {
            newIngs.forEach(ing => ingMap.set(ing.name.toLowerCase(), ing.id))
          }
        }

        // 3. Construct and bulk insert many-to-many bridge records
        // We iterate over the original validIngredients array (including duplicates)
        // because a recipe might legitimately have the same ingredient twice (e.g. separated for different steps)
        const bridgePayloads = validIngredients
          .map(ing => {
            const ingredientId = ingMap.get(ing.name.trim().toLowerCase())
