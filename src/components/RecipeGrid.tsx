      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md px-4 pt-4 pb-2">
        <div className="flex gap-2">
          ...
          <button 
            ...
            title="Surprise Me"
            className="shrink-0 w-12 h-12 bg-white border border-border rounded-md flex items-center justify-center text-text-hi hover:bg-stone-50 transition-colors shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-stone-300"
          >
            <Dices className="w-6 h-6" aria-hidden="true" />
          </button>
          <button 
            onClick={() => setIsLazyFilter(!isLazyFilter)}
            aria-label="Toggle I'm Lazy Mode filter"
            aria-pressed={isLazyFilter}
            title="I'm Lazy Mode"
            className={`shrink-0 w-12 h-12 border border-border rounded-md flex items-center justify-center transition-colors shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-stone-300 ${
