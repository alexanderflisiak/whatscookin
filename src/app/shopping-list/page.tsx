    ...
    <div className="max-w-xl mx-auto p-4 pb-24">
      ...
      <form onSubmit={handleAdd} className="flex gap-2 mb-8">
        <input 
          ...
          className="flex-1 p-3 border border-border rounded-xl bg-surface focus:ring-2 focus:ring-primary/50 outline-none font-medium shadow-sm"
        />
        <button 
          type="submit" 
          disabled={!newItem.trim()}
          className="w-12 h-12 shrink-0 bg-primary text-white rounded-xl flex items-center justify-center hover:bg-orange-500 disabled:opacity-50 transition-colors shadow-sm"
        >
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {activeItems.length === 0 ? (
            <div className="py-6 text-center text-text-lo text-sm bg-surface rounded-xl border border-dashed border-border opacity-75">
              List is empty. You're all caught up!
            </div>
          ) : (
            <div className="space-y-2">
              {activeItems.map(item => (
                <div key={item.id} className="flex items-center gap-3 bg-surface p-3 pr-4 rounded-xl border border-border shadow-sm group">
                  <button 
                    onClick={() => toggleBought(item.id, item.is_bought)}
                    className="w-8 h-8 rounded-full border-2 border-primary/50 flex items-center justify-center hover:bg-orange-50 transition-colors shrink-0"
                  >
            ...
            <div className="space-y-1">
              {boughtItems.map(item => (
                <div key={item.id} className="flex items-center gap-3 py-2 px-1 group">
                  ...
                  <button 
                    onClick={() => toggleBought(item.id, item.is_bought)}
                    className="text-text-lo hover:text-primary p-1 flex items-center gap-1 text-xs font-bold"
                  >
