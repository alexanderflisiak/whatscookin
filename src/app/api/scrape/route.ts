...
async function isUrlSafe(urlString: string): Promise<boolean> {
  try {
    ...
    if (hostname.endsWith('.local') || hostname.endsWith('.internal') || hostname === 'localhost') return false

    // Resolve DNS to prevent SSRF bypass via custom domains pointing to local IPs
    const { address } = await dns.lookup(hostname).catch(() => ({ address: hostname }))

    const checkIp = (ip: string) => {
      // Block localhost and 0.0.0.0
      if (ip === '0.0.0.0' || ip === '::1' || ip === '::' || ip === '0:0:0:0:0:0:0:0' || ip === '0:0:0:0:0:0:0:1') return false

      // Block private IP ranges (IPv4)
      if (ip.startsWith('10.') || ip.startsWith('192.168.') || ip.startsWith('127.') || ip.startsWith('169.254.')) return false
    ...
    return checkIp(address) && checkIp(hostname)
  } catch {
