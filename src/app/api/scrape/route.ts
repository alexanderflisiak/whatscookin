...
function isIpBlocked(ipStr: string): boolean {
  // Block localhost and 0.0.0.0
  if (ipStr === 'localhost' || ipStr === '0.0.0.0') return true

  // Block IPv6 localhost and unspecified
  if (ipStr === '::1' || ipStr === '::' || ipStr === '0:0:0:0:0:0:0:0' || ipStr === '0:0:0:0:0:0:0:1') return true

  // Block private IP ranges (IPv4)
  // 10.0.0.0 - 10.255.255.255
  if (ipStr.startsWith('10.')) return true
  // 172.16.0.0 - 172.31.255.255
  if (/^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(ipStr)) return true
  ...
  if (/^(fc|fd|fe8|fe9|fea|feb)[0-9a-f]{0,2}:/i.test(ipStr)) return true

  // Block IPv4-mapped IPv6 addresses
  const lowerHost = ipStr.toLowerCase()
...
async function isUrlSafe(urlString: string): Promise<boolean> {
  try {
    ...
    // Fast fail for raw IP inputs
    if (isIpBlocked(hostname)) return false

    // Block internal domains based on TLD
    if (hostname.endsWith('.local') || hostname.endsWith('.internal')) return false

    // Perform DNS lookup for DNS rebinding protection
    const addresses = await dns.promises.lookup(hostname, { all: true })

    // If ANY of the resolved IPs are in a private/blocked range, block the request
    ...
    return false // Invalid URL format or DNS resolution failed
  }
}
