...
async function isUrlSafe(urlString: string): Promise<boolean> {
  try {
    ...
    // Strip brackets for IPv6 literal lookup
    if (hostname.startsWith('[') && hostname.endsWith(']')) {
      hostname = hostname.slice(1, -1)
    }

    // DNS resolution to prevent DNS rebinding attacks
    // We lookup all IPs to ensure none of the resolved IPs are internal
    try {
      const lookups = await dns.promises.lookup(hostname, { all: true })
      for (const lookup of lookups) {
        if (!isHostSafe(lookup.address)) return false
      }
    } catch {
      // If DNS resolution fails, it's safer to reject the URL
      return false
    }
