...
function isHostSafe(hostname: string): boolean {
  ...
  if (host === '::1' || host === '::' || host === '0:0:0:0:0:0:0:0' || host === '0:0:0:0:0:0:0:1') return false
  ...
  if (/^(fc|fd|fe8|fe9|fea|feb)[0-9a-f]{0,2}:/i.test(host)) return false
...
async function isUrlSafe(urlString: string): Promise<boolean> {
  try {
    ...
    // First validate the hostname string itself
    if (!isHostSafe(hostname)) {
      return false
    }

    // Strip brackets before DNS lookup
    ...
    // dns.promises.lookup throws ENOTFOUND if it can't resolve, block in this case
    return false
  }
}
