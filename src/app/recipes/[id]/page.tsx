...
export default function RecipeDetailPage() {
  ...
  return (
    <div className="pb-32 bg-background min-h-full relative">
      <Link href="/" aria-label="Go back" className="absolute top-4 left-4 z-20 w-10 h-10 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-black/50 transition-colors focus-visible:ring-2 focus-visible:ring-white outline-none">
      ...
      <div className="px-5 py-6">
        <div className="flex items-center justify-between mb-8 border-b border-border/60 pb-6">
          ...
          <Link href={`/recipes/${recipe.id}/edit`} aria-label="Edit recipe" className="w-10 h-10 rounded-full bg-stone-100 flex items-center justify-center text-text-hi hover:bg-stone-200 transition-colors shrink-0 focus-visible:ring-2 focus-visible:ring-stone-400 outline-none">
        ...
        <div className="mb-10">
          ...
          <div className="space-y-1">
            {ingredients.map((ing) => {
              ...
              return (
                <button key={ing.ingredient_id} onClick={() => toggleCheck(ing.ingredient_id)} aria-pressed={isChecked} className="w-full flex items-center gap-4 py-3 min-h-[48px] px-2 -mx-2 hover:bg-stone-100 rounded-md transition-colors group text-left focus-visible:ring-2 focus-visible:ring-stone-400 outline-none">
