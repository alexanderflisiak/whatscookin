...
## 2024-07-20 - [Memory Exhaustion DoS via Scraper]
**Vulnerability:** The scraper endpoint used `response.text()` to load the entire HTTP payload into memory unconditionally. An attacker providing a URL pointing to an infinitely large or gigabyte-sized payload could crash the Node server via OOM (Out-of-Memory) errors.
**Learning:** In public-facing proxies or scrapers, you can never trust the target server to provide a reasonably sized response. Using `response.text()` or `response.arrayBuffer()` is a severe DoS risk.
**Prevention:** Always use `response.body.getReader()` to stream the response, decode it incrementally, and enforce a strict maximum byte limit. If the limit is reached, invoke `reader.cancel()` immediately to abort the stream and free up bandwidth.
