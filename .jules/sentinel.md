...
## 2024-06-03 - OOM DoS via response.text()
**Vulnerability:** Calling `await response.text()` on an arbitrary external URL without checking the size.
**Learning:** External servers can return infinitely large responses (e.g. gzip bombs or massive files) which will be fully loaded into memory, causing an Out-Of-Memory (OOM) crash on the server.
**Prevention:** Use streaming with `response.body.getReader()` to incrementally process the response and strictly enforce a maximum byte limit, ensuring `await reader.cancel()` is called if the limit is exceeded.
