...
## 2026-06-20 - Prevent DoS via memory exhaustion in scraping
**Vulnerability:** The `/api/scrape` endpoint read entire remote files into memory using `response.text()`, allowing memory exhaustion via unbounded payloads.
**Learning:** When fetching untrusted URLs, streaming the response instead of fully buffering it is critical to prevent attackers from sending massive responses to crash the server.
**Prevention:** Use `response.body.getReader()` to read data in chunks, enforcing a strict byte limit and cancelling the stream if exceeded.
