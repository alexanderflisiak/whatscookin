...
## 2024-06-23 - Prevent DoS via Memory Exhaustion in URL Scraper
**Vulnerability:** The scraper endpoint used `await response.text()` to load untrusted HTTP responses into memory without a size limit.
**Learning:** Loading unbounded network payloads directly into memory risks Denial-of-Service via Out-Of-Memory (OOM) crashes if a malicious server sends an endless stream or extremely large payload.
**Prevention:** Always read untrusted network responses via streams (`response.body.getReader()`) and enforce a hard byte limit, calling `reader.cancel()` and aborting if exceeded.
