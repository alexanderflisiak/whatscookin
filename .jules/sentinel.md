...
## 2026-04-14 - [IDOR in Shopping List Updates]
**Vulnerability:** Client-side updates and deletions in the shopping list relied only on the item's ID, potentially allowing users to modify others' lists.
**Learning:** Relying solely on RLS requires correctly configured policies. Client-side mutations should enforce row-level ownership locally via query filters.
**Prevention:** Always include an `.eq('created_by', currentUserId)` filter on update and delete operations.
