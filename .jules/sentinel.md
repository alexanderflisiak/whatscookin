...
## 2026-07-02 - SSRF DNS Resolution bypass
**Vulnerability:** The scraper API validated URL safety by checking the hostname text, which could be bypassed using custom domains pointing to local IPs (like localtest.me).
**Learning:** String-matching on hostnames is insufficient for SSRF protection because DNS can resolve external-looking domains to internal IP addresses.
**Prevention:** Always perform DNS resolution (e.g., `dns.promises.lookup`) and validate the resolved IP address before making network requests in server environments.
