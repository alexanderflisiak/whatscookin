...
## 2024-05-20 - DNS Rebinding SSRF bypass
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass via DNS Rebinding.
**Learning:** Checking the hostname string of a URL against a blocklist is insufficient to prevent SSRF because a malicious user can supply a domain (like `localtest.me`) that safely passes string checks but resolves to a local/private IP address (like `127.0.0.1`) when fetched by the server.
**Prevention:** To prevent DNS Rebinding, you must asynchronously resolve the hostname using `dns.promises.lookup(hostname, { all: true })` and validate EVERY resolved IP address against the explicit blocklist before initiating the fetch. Make sure to strip `[]` from IPv6 hostnames before calling lookup.
