...
## 2026-06-25 - Prevent DoS via memory exhaustion in web scraper
**Vulnerability:** Scraper loaded untrusted URLs directly into memory using `response.text()`, risking DoS (OOM) for large payloads.
**Learning:** Malicious sites can return indefinitely large payloads. We must always bound resource consumption when fetching user-provided URLs.
**Prevention:** Use a streaming reader (`response.body.getReader()`) and enforce a hard byte limit, calling `cancel()` if exceeded.
