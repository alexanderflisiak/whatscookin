...
## 2024-07-07 - DNS Rebinding SSRF
**Vulnerability:** TOCTOU DNS rebinding in scrape API. URL was checked before fetch, but fetch resolves DNS internally, allowing an attacker to rebind to a local IP.
**Learning:** Relying on pre-fetch URL validation fails if the attacker controls the DNS and changes the IP to a forbidden one during the actual connection. Replacing fetch host with IP breaks SNI.
**Prevention:** Use http/https module with a custom Agent that intercepts the DNS lookup, checks the resolved IP, and aborts if unsafe.
