...
## 2024-10-18 - [Missing Authentication on API Scrape Endpoint]
**Vulnerability:** The `/api/scrape` endpoint was completely unauthenticated. While standard routes were protected by Supabase middleware, API routes (`/api/*`) are explicitly excluded from these middleware checks to prevent CORS preflight issues or to support public APIs.
**Learning:** In Next.js App Router architectures using Supabase SSR, relying solely on global middleware for protection is insufficient if API paths are excluded.
**Prevention:** Always manually instantiate `createClient` from `@/lib/supabase/server` within sensitive API route handlers and explicitly check `await supabase.auth.getUser()`, returning a 401 response if no user session is present.
