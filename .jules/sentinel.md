...
## 2024-07-28 - [Critical DoS vulnerability via OOM in Scraping Endpoint]
**Vulnerability:** The `/api/scrape` route used `await response.text()` which loaded the entire response body into memory, leaving the server vulnerable to Denial of Service via memory exhaustion (OOM) if an attacker provided a URL returning a massive payload.
**Learning:** When fetching untrusted external URLs, native fetch provides no implicit size limit. Using methods like `.text()` or `.arrayBuffer()` will eagerly buffer arbitrary sizes.
**Prevention:** Implement a streaming reader via `response.body.getReader()`, enforcing a hard byte limit and explicitly calling `await reader.cancel()` if the limit is exceeded to prevent resource leaks.
